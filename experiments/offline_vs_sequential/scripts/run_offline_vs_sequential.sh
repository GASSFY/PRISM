...
#   bash experiments/offline_vs_sequential/scripts/run_offline_vs_sequential.sh
set -euo pipefail
...
OUT_ROOT="${OUT_ROOT:-experiments/offline_vs_sequential}"
TASKS="${TASKS:-mmmu_val,realworldqa,ocrbench,ai2d}"
RESULTS_MD="${OUT_ROOT}/results/compare_results.md"

mkdir -p "$OUT_ROOT/scale_cache" "$OUT_ROOT/logs" "$OUT_ROOT/results"
