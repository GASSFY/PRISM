#!/usr/bin/env python3
"""Build one offline pseudo-quant checkpoint via worst-modality greedy keep.

No searchable parameter. Gains g^m = globally-normalized K^m.
Each round rescues argmax_m (G^m - C^m) until |Ω|=B.
"""
...
from prism.quantization.mixed_precision import select_columns_worst_modality_greedy
...
def main() -> None:
    ...
    print(f"[s2] load model {cfg['model']} (worst-modality greedy)", flush=True)
    ...
    t0 = time.perf_counter()
    print("[s2] load calib", flush=True)
    ...
    if args.energy_cache and os.path.exists(args.energy_cache):
        print(f"[s2] load energy cache {args.energy_cache}", flush=True)
        t0 = time.perf_counter()
        ...
        print(f"[s2] energy done {timing['seconds_energy']:.1f}s", flush=True)
    ...
    print(f"[s2] CE_fp={ce_fp:.6f} ({timing['seconds_fp_ce']:.1f}s)", flush=True)
    ...
    t0 = time.perf_counter()
    hp_cols = select_columns_worst_modality_greedy(
        process_model.model,
        energy["vision"],
        energy["text"],
        ratio=ratio,
    ...
    print(
        f"[s2] kept={len(hp_cols)} select={timing['seconds_select']:.2f}s",
        flush=True,
    )
    ...
    pseudo_quantize_model_weight(
        ...
        low_w_bit=low_w_bit,
        progress_label="s2_greedy",
    )
    timing["seconds_pseudo_quant"] = time.perf_counter() - t0
    print(f"[s2] pseudo-quant done {timing['seconds_pseudo_quant']:.1f}s", flush=True)

    t0 = time.perf_counter()
    ce_q = compute_ce(process_model, all_kw)
    timing["seconds_ce_q"] = time.perf_counter() - t0
    delta = ce_q - ce_fp
    print(f"[s2] CE_q={ce_q:.6f} DeltaCE={delta:.6f}", flush=True)
    ...
    metrics: dict[str, Any] = {
        ...
        "formula": "greedy: m*=argmax(G^m-C^m), c*=argmax g^{m*} (tie-break other)",
    ...
    print("[s2] metrics written", args.metrics_json, flush=True)
    print(f"[s2] timing={json.dumps(timing)}", flush=True)
